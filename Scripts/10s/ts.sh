...
# 环境变量定义
readonly RESULT_PATH="/home/ubuntu20/Workspace/Datasets/Image_poison/10s/tsc"  # 结果保存路径
...
# 日志输出函数,输出带时间戳的日志信息
log_msg() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') - $1" | tee -a /home/ubuntu20/Workspace/Datasets/Image_poison/10s/tsc.txt
}
